...
public class App {
    ...
    public static void main(String[] args) {
        ...
        List<Integer> crabPositions = Arrays.stream(inputReader.readFile().split(","))
                .map(Integer::parseInt)
                .toList();
        int maxPosition = crabPositions.stream().mapToInt(v -> v).max().getAsInt();

        int[] modes = new int[maxPosition+1];
        crabPositions.stream().forEach(p -> modes[p]++);
        int modiest = 0;
        int mode = 0;
        ...
        int avg = (int) Math.round(crabPositions.stream().mapToInt(v->v).average().getAsDouble());
        ...
        System.out.println(String.format("Average: %f",crabPositions.stream().mapToInt(v->v).average().getAsDouble()));
    }
}
